...
class ViTAdapter(nn.Module):
    def __init__(self, conv_inplane=64, n_points=4, deform_num_heads=16,
                 init_values=1e-6, with_cffn=True, cffn_ratio=0.25,
                 deform_ratio=0.5, add_vit_feature=True, use_extra_extractor=True,with_cp=True,drop_path_rate=0.4,img_size: int = 1024,
                 patch_size: int = 16, in_chans: int = 3, embed_dim: int = 768, depth: int = 12,
                 num_heads: int = 12, mlp_ratio: float = 4.0, out_chans: int = 256, qkv_bias: bool = True,
                 norm_layer: Type[nn.Module] = nn.LayerNorm, act_layer: Type[nn.Module] = nn.GELU, use_abs_pos: bool = True,
                 use_rel_pos: bool = False, rel_pos_zero_init: bool = True, window_size: int = 0,
                 global_attn_indexes: Tuple[int, ...] = (), frozen_stages = 32) -> None:
        super().__init__()
        ...
        self.interactions = nn.Sequential(*[
            InteractionBlock_efficient(dim=embed_dim, num_heads=deform_num_heads, n_points=n_points,
    ...
    def forward(self, x):
        ...
        x = self.patch_embed(x)  #torch.Size([bs, 64, 64, c])
        # print(x.shape)
        bs, H, W, dim = x.shape
        ...
        for i, layer in enumerate(self.interactions):
            ...
            outs_multilayer.append(x.transpose(1, 2).view(bs, dim, H, W).contiguous())
        ...
        c2 = c2.transpose(1, 2).view(bs, dim, H * 2, W * 2).contiguous()  #bs,dim,128,128
        c3 = c3.transpose(1, 2).view(bs, dim, H, W).contiguous()   #bs,dim,64,64
        c4 = c4.transpose(1, 2).view(bs, dim, H // 2, W // 2).contiguous()  ##bs,dim,32,32
        c1 = self.up(c2) + c1  ##bs,dim,256,256

        if self.add_vit_feature:
            x1, x2, x3, x4 = outs_multilayer
            x1_1 = F.interpolate(x1, scale_factor=4, mode='bilinear', align_corners=False)
            x2_1 = F.interpolate(x2, scale_factor=2, mode='bilinear', align_corners=False)
            x4_1 = F.interpolate(x4, scale_factor=0.5, mode='bilinear', align_corners=False)
            c1, c2, c3, c4 = c1 + x1_1, c2 + x2_1, c3 + x3, c4 + x4_1
    ...
    def train(self, mode=True):
        ...
        super(ViTAdapter, self).train(mode)
        self._freeze()


class D2ImageEncoderViTAdapter(ViTAdapter, Backbone):
    def __init__(self, cfg, input_shape):
        img_size = cfg.MODEL.VIT.IMAGE_SIZE
        patch_size = cfg.MODEL.VIT.VIT_PATCH_SIZE
        in_chans = 3
        embed_dim = cfg.MODEL.VIT.EMBED_DIM
        depth = cfg.MODEL.VIT.DEPTH
        num_heads = cfg.MODEL.VIT.NUM_HEAD
        mlp_ratio = 4
        out_chans= cfg.MODEL.VIT.CONVS_DIM
        qkv_bias = True
        ...
        global_attn_indexes = cfg.MODEL.VIT.ENCODER_GLOBAL_ATTN_INDEXES
        frozen_stages = cfg.MODEL.VIT.FROZEN_STAGES
        super().__init__(
            img_size=img_size,
